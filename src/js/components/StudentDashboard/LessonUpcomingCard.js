...
const LessonUpcomingCard = ({
 ...
 onHandleRequireLesson,
  ...
  const handleRequireLesson = (id, avatar, teacher, name, note, date, start, end, documents, skype) => {
    onHandleRequireLesson(id, avatar, teacher, name, note, date, start, end, documents, skype)
  }

  const handleCancelBooking = (e, id, name, date, start, end) => {
    e.preventDefault()
    onHandleCancelBooking(id, name, date, start, end)
  }
  return (
    ...
    <a href="lessonDetail.html" className="course-name tx-bold">{name}</a>
    </h5>
    ...
    <a href={skype} className="btn btn-sm btn-info mg-r-10" target="_blank"
    ...
    onClick={(e)=>handleCancelBooking(e, id, name, date, start, end)}
