...
const RenderRow = ({ data }) => {
    return (<tr>
        <td data-title="Salery period" className="tx-left wd-md-150">{data.SalaryPeriod}</td>
        ...
        <td data-title="Date" className="tx-center">{data.CreatedDate}</td>
                        ...
                        <tbody>
                            {isLoading ? (
                                <>
                                    ...
                                    !!data && !!data.length > 0 ? data.map((item, index) => <RenderRow key={`${index}`} data={item} />) : (<tr><td colSpan={4}><span className="tx-danger d-block tx-center tx-medium tx-16">No history record.</span></td></tr>)
                                )}
